...
interface ParkingSpaceSnapshot {
  id: string;
  pricePerHour: number;
...
export class ReservationsService {
  ...
  private async fetchParkingSpace(parkingSpaceId: string): Promise<ParkingSpaceSnapshot> {
    const parkingServiceUrl = this.configService.get<string>('PARKING_SERVICE_URL');

    if (!parkingServiceUrl) {
      throw new BadRequestException('PARKING_SERVICE_URL is not configured');
    }

    const response = await firstValueFrom(
      this.httpService.get<ApiResponse<ParkingSpaceSnapshot>>(`${parkingServiceUrl}/parking-spaces/${parkingSpaceId}`),
    );
  ...
  private async createNotification(
    ...
    if (!notificationServiceUrl || !authorizationHeader) {
      return;
    }
