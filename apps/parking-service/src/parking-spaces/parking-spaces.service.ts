...
interface ParkingSpaceWithDistance extends ParkingSpaceRecord {
  distanceKm?: number;
}
...
export class ParkingSpacesService {
  ...
  private async geocodeAddress(address: string): Promise<GeocodeResult> {
    const mapsServiceUrl = this.configService.get<string>('MAPS_SERVICE_URL');

    if (!mapsServiceUrl) {
      throw new NotFoundException('MAPS_SERVICE_URL is not configured');
    }

    const response = await firstValueFrom(
      this.httpService.get<ApiResponse<GeocodeResult>>(`${mapsServiceUrl}/maps/geocode`, {
        params: { address },
      }),
    );
